...
We are 95% confident the true mean battery lifetime is between 469.36 and 504.64 hours.

\(e\) Build a 99% interval. Is it wider or narrower, and why?

```{r}
margin_99 <- qnorm(0.995) * se

487 - margin_99
487 + margin_99
```

The 99% interval runs from 463.82 to 510.18 hours, so it is wider. More confidence requires covering more values, which means a larger critical value (2.576 instead of 1.96).
...
We reject if $z < -1.96$ or $z > 1.96$.

\(d\) Is the test statistic in the rejection region? State your conclusion.

The test statistic is -2.25, which is below -1.96, so it falls in the rejection region. We reject $H_0$ and conclude the mean fill weight has shifted away from 340 grams.
...
The p-value is 0.024, which is below 0.05, so we reject $H_0$. This matches part (d).
...
$$H_0: \mu = 24 \qquad H_a: \mu > 24$$

\(b\) Compute the sample mean and the standard error.

```{r}
ybar_j <- mean(jumps)
se_j <- 1.2 / sqrt(length(jumps))
...
The test statistic of 3.74 is greater than the critical value of 1.645, so we reject $H_0$. There is evidence the mean vertical jump is above 24 inches.

\(e\) Compute the one-tailed p-value.

```{r}
pnorm(z_j, lower.tail = FALSE)
```

The p-value is about 0.00009, far below 0.05, which agrees with part (d).

\(f\) Build a 95% confidence interval for the mean vertical jump.

```{r}
ybar_j - qnorm(0.975) * se_j
ybar_j + qnorm(0.975) * se_j
```

We are 95% confident the true mean vertical jump is between 24.55 and 25.77 inches.
