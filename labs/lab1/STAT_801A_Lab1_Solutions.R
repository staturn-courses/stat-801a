...
head(mtcars) # The head function prints the first six observations
str(mtcars)
nrow(mtcars)
ncol(mtcars)
names(mtcars)
...
student_data$test <- student_data$study_hours * student_data$quiz_score
...
student_data <- read.csv("C:\\Users\\baren\\OneDrive\\Desktop\\801Lab\\Student_data.csv")
...
cereal[cereal$sugar_g > 10 & cereal$fat_g < 3,]
...
cereal$high_sodium <- cereal$sodium_mg > 200


# How many cereals are high sodium?

sum(cereal$high_sodium)


# 7. ADDITIONAL CHALLENGES

# Use ?table to help figure out how many cereals are on each shelf.

table(cereal$Shelf)

# Use ?unique to help find the unique values of the Shelf variable.

unique(cereal$Shelf)

# Display all cereals with between 5 and 10 grams of sugar.

cereal[cereal$sugar_g >= 5 & cereal$sugar_g <= 10, ]

# What is the average sugar content of cereals on Shelf 1?

mean(cereal$sugar_g[cereal$Shelf == 1])
