...
\begin{frame}
\frametitle{ Properties of Probability Distributions of Discrete Random Variable X}
\begin{itemize}
\item  $0\leq P(X=x)\leq 1$ for all x
\item $\sum_{x}P(X=x)=1$
\end{itemize}
\end{frame}
...
with $Z$ which are available from the Standard Normal Probabilities table.     
\end{itemize}
\end{frame}
...
\item The \textcolor{magenta}{standard normal quantiles} are the $z$-values found from the
Standard Normal Probabilities table for specified $p$ values that are in the body of the table.
\end{itemize}
\end{frame}


\begin{frame}
\frametitle{Examples:  Using  the
...
\item We know $P(-z_{\frac{0.05}{2}} \leq Z \leq z_{\frac{0.05}{2}}) =
         0.95$ and $ z_{0.025} = 1.96$ \quad from the Standard Normal Probabilities Table
\end{itemize}
\end{frame}
...
d.) $P(Z < a) = 0.90$ From the Standard Normal Probabilities Table,  a = 1.28 \\[.1in]
...
\item Certain conditions must be met regarding the occurrence of these events and the definition of the period of time or region of space in which they occur for a Poisson random variable to represent the number of events. 
